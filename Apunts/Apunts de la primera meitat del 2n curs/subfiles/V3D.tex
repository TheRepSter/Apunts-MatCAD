...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{obs}
			El fet de no saber la solució llevat de multiple és perfectament normal. (pel terorema de Tales, si $d$ es distancia entre les cameres i $h$ l'altura de l'objecte, si canviem la distancia per $\lambda d$ i l'altura per $\lambda h$, la imatge no canvia).
		\end{obs}
		...
		\begin{demostracio}
			\begin{enumerate}
				...
				i deduim el resultat com $\hat{T}^2 = \begin{psmallmatrix} -T_3^2-T_2^2 & T_1T_2 & T_1T_3\\T_1T_2 & -T_3^2-T_1^2 & T_2T_3\\T_1T_3 & T_2T_3 & -T_2^2-T_1^2\end{psmallmatrix}$ i la traça només agafa la diagonal.
			\end{enumerate}
		\end{demostracio}
		Aleshores, com hem suposat que $n = 8$, rang($A$) $= 8$ i $||T|| = 1$ podem determinar $E$ llevat de múltiple per un escalar, pero amb tr$(EE^t) = 2$ podem determinar $E$ llevat d'un signe $\pm$. Triem un dels dos signes.
		\begin{obs}
		...
		\end{demostracio}
		PARENTESI: Si tenim $(U, V, W)$ BON $\oplus$ a $\mathbb{R}^3$, i $A = \begin{psmallmatrix}U & V & W\end{psmallmatrix} \in SO(3)$. Per què?\\
		...
		\item L'ultim pas consisteix en determinar les coordenades a $\mathbb{R}^3$ d'un punt $P$ (qualsevol) donat les seves imatges.\\
