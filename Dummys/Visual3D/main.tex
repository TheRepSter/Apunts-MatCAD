...
\begin{document}
	...
	\subsection{Definició i primeres propietats:}
	...
	\begin{teorema}[Relació epipolar]
	...
	\begin{enumerate}
		...
		\begin{obs}
			El fet de no saber la solució llevat de múltiple és perfectament normal. (pel teorema de Tales, si $d$ es distancia entre les càmeres i $h$ l'altura de l'objecte, si canviem la distància per $\lambda d$ i l'altura per $\lambda h$, la imatge no canvia).
		\end{obs}
		En canvi, si coneixem una de les distàncies entre dos dels 8 punts $P_1,\dots, P_8$, podem determinar aquest $\lambda$ i llavors determinar $E$ sense suposar $||T|| = 1$.
		\begin{proposicio}
			Sigui $E = \hat{T}R$ una matriu essencial. ($T \subset \mathbb{R}^3,\;R\in SO(3)$), aleshores
			\begin{enumerate}
				\item Ker($E^t$) $=$ Vect($T$)
				\item Tr($EE^t$)$= 2 ||T||^2$
			\end{enumerate}
		...
		\begin{demostracio}
			\begin{enumerate}
				\item $X \in \text{Ker}(E^t) \Leftrightarrow E^tX = 0 \Leftrightarrow (\hat{T}R)^tX = 0 \Leftrightarrow \hat{T}X = 0 \iff T\wedge X = 0 \Leftrightarrow X \in \text{Vect}(T)$
				\item $\text{Tr}(EE^t) = -\text{Tr}(\hat{T}RR^t\hat{T}) =  -\text{Tr}(\hat{T}^2)$
				i deduïm el resultat com $\hat{T}^2 = \begin{psmallmatrix} -T_3^2-T_2^2 & T_1T_2 & T_1T_3\\T_1T_2 & -T_3^2-T_1^2 & T_2T_3\\T_1T_3 & T_2T_3 & -T_2^2-T_1^2\end{psmallmatrix}$ i la traça només agafa la diagonal.
			\end{enumerate}
		\end{demostracio}
		Aleshores, com hem suposat que $n = 8$, rang($A$) $= 8$ i $||T|| = 1$ podem determinar $E$ llevat de múltiple per un escalar, però amb tr$(EE^t) = 2$ podem determinar $E$ llevat d'un signe $\pm$. Triem un dels dos signes.
		\begin{obs}
			Si $E$ descriu la rotació entre les dues càmeres: $X^tEY = 0$ aleshores la matriu $-E$ també: $X^t(-E)Y = 0$.
		\end{obs}
		\item En el segon pas, determinem la posició relativa $(R, T)$ entre les dues cameres. Acabem de determinar $E$ i per tant podem triar $T \in \text{Ker}(E^t)$ de norma $1$ que està determinat llevat d'un signe, triem un dels signes.
		\begin{obs}
		...
		\begin{demostracio}
			...
			Si posem $A := \begin{psmallmatrix}V & -U & -T\end{psmallmatrix}$ com $(U, T, V)$ és BON $\oplus$ veiem que $A$ és BON $\oplus$ i doncs $A \in SO(3) \Rightarrow A^{-1} = A^t$\\
			Al final tenim $A^{-1}R = \begin{psmallmatrix}E^tU & E^tV & E^tU \wedge E^tV\end{psmallmatrix}^t \Rightarrow R = A\begin{psmallmatrix}E^tU & E^tV & E^tU \wedge E^tV\end{psmallmatrix}^t$
		\end{demostracio}
		PARÈNTESI: Si tenim $(U, V, W)$ BON $\oplus$ a $\mathbb{R}^3$, i $A = \begin{psmallmatrix}U & V & W\end{psmallmatrix} \in SO(3)$. Per què?\\
		$A^tA = \begin{psmallmatrix}U^t \\ V^t \\ W^t\end{psmallmatrix}\begin{psmallmatrix}U&V&W\end{psmallmatrix} = \begin{psmallmatrix}U^tU & U^tV & U^tW\\V^tU & V^tV & V^tW\\W^tU & W^tV & W^tW\end{psmallmatrix} = \begin{psmallmatrix} \left\langle U, U\right\rangle & \left\langle U, V\right\rangle & \left\langle U, W\right\rangle\\\left\langle V, U\right\rangle & \left\langle V, V\right\rangle & \left\langle V, W\right\rangle\\\left\langle W, U\right\rangle & \left\langle W, V\right\rangle & \left\langle W, W\right\rangle\end{psmallmatrix} = I_3$\\
		\item L'últim pas consisteix en determinar les coordenades a $\mathbb{R}^3$ d'un punt $P$ (qualsevol) donant les seves imatges.\\
		Tenim un punt $P = (X, Y, Z) \in \mathbb{R}^3$ tal que $I_1(P) = \begin{psmallmatrix}x_1\\y_1\\1\end{psmallmatrix}$ i $I_2(P) = \begin{psmallmatrix}x_2\\y_2\\1\end{psmallmatrix}$
